...
ax.set_ylabel('Total savings vs P_floor (thousand CNY)')
ax.spines['top'].set_visible(False);ax.spines['right'].set_visible(False)
fig.suptitle('Monthly savings include losing months',fontsize=19,fontweight='bold',y=.96)
fig.legend(*ax.get_legend_handles_labels(),loc='upper center',bbox_to_anchor=(.54,.89),ncol=4,frameon=False,fontsize=11)
fig.text(.1,.065,'Positive = lower actual total bill. Best observed eligible profile is selected after historical replay.',fontsize=11)

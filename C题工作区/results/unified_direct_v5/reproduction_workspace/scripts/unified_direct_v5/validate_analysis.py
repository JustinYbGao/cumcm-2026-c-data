...
def main():
    ...
    table=read(ROOT/'comparison_all.csv');daily={};costerr=0.;physics=[]
    for _,row in table.iterrows():
        ...
        assert np.all(np.isfinite(f.select_dtypes(include='number').to_numpy()))
    ...
    result={'passed':True,'comparison_policies':len(table),'source_fee_max_error_yuan':costerr,'paired_statistics_max_error_yuan':staterr,'paired_comparisons':len(summary),'physics':physics,'fixed_planning_cross_price_consistency':same,'bootstrap':'Independent loop reconstruction, seed20260915/2000 circular blocks7,14, same frozen pair order'}

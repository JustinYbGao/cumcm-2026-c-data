...
def raw_prices() -> tuple[dict[int, float], dict[tuple[str, int], float], dict[str, str]]:
    ...
    with openpyxl.load_workbook(p1, read_only=True, data_only=True) as wb:
        ws = wb[wb.sheetnames[0]]
        fixed = {slot: float(ws.cell(slot + 1, 2).value) for slot in range(1, 145)}
    with openpyxl.load_workbook(p4, read_only=True, data_only=True) as wb:
        ws = wb[wb.sheetnames[0]]
        variable = {}
        for row in range(2, 367):
            date = date_text(ws.cell(row, 1).value)
            for slot in range(1, 145):
                variable[(date, slot)] = float(ws.cell(row, slot + 1).value)
...
def raw_actual_energy() -> tuple[dict[tuple[str, int], tuple[float, float]], str]:
    ...
    with openpyxl.load_workbook(path, read_only=True, data_only=True) as wb:
        load, pv = wb["小区负载"], wb["光伏发电实际功率"]
        for row in range(2, 367):
            date = date_text(load.cell(row, 1).value)
            for slot in range(1, 145):
                result[(date, slot)] = (float(load.cell(row, slot + 1).value) / 6.0, float(pv.cell(row, slot + 1).value) / 6.0)
...
def validate_mapping(audit: Audit) -> None:
    ...
    for filename, sheets in {
        ...
        with openpyxl.load_workbook(TEMPLATES / filename, read_only=True, data_only=False) as wb:

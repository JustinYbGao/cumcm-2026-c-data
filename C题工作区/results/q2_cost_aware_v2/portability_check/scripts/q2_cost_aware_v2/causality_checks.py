...
if __name__ == '__main__':
    ...
    folder = ROOT / ('causality_extension_inputs' if args.extension else 'causality_inputs'); folder.mkdir(exist_ok=True)
    ...
    kind='seasonal' if args.extension else 'linear_harmonic'
    ...
    checks = []
    for cfg in json.loads((WORK/'configs/q2_cost_aware_v2'/('extension.json' if args.extension else 'experiments.json')).read_text())['runs']:
        ...
        pa=pd.read_csv(ROOT/'causality_original'/cfg['policy']/'plans.csv',float_precision='round_trip',dtype={'selected_terminal':str})
        pb=pd.read_csv(ROOT/'causality_mutated'/cfg['policy']/'plans.csv',float_precision='round_trip',dtype={'selected_terminal':str})
        ...
        ea=np.load(ROOT/'causality_original'/cfg['policy']/'decision_evidence.npz')
        eb=np.load(ROOT/'causality_mutated'/cfg['policy']/'decision_evidence.npz')
    ...
    evidence=np.load(ROOT/'january/S_joint/decision_evidence.npz')
    ...
    np.savez_compressed(folder/'scenario_prefix_evidence.npz',grid=grid,net_original=net_a,net_mutated=net_b,price=price,
    ...
    save_json(WORK/'reports/q2_cost_aware_v2'/('causality_extension_validation.json' if args.extension else 'causality_validation.json'),result)

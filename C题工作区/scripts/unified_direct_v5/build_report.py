...
def main():
    ...
    assert checks['workbook']['coverage']['workbooks']==5
    assert {v['policy'] for v in checks['reproduction']['policies']}=={chosen[b] for b in ['q3','q42','q43']}
    ...
    matched=[]
    for name,old in [('q3_raw','old_q3_raw'),('q42_fixed','old_q42_fixed'),('q43_raw_ols','old_q43_raw_ols')]:
        c,r=lookup.loc[name],lookup.loc[old]
        matched.append({'新策略':name,'同PV/规划价旧策略':old,'总费节省/元':r.total_cost_yuan-c.total_cost_yuan,
            '合同费节省/元':r.contract_cost_yuan-c.contract_cost_yuan,'紧急费节省/元':r.emergency_cost_yuan-c.emergency_cost_yuan})
    ...
    paragraphs=[
      ...
      '', '## 最终选择与真实费用', '',
      markdown(pd.DataFrame(selected_rows)),
      '',
      ...
      '',
      '除了与此前主策略比较，以下给出相同PV版本和规划价格输入下的新旧对照。负的合同费节省表示承诺采购更贵；正的紧急费节省超过该增量才带来总费下降。它们仍包含目标、场景、终态与采购集合的组合变化。', '', markdown(pd.DataFrame(matched)),
      '', '## 十条新策略与失败结果', '',
      markdown(new[['policy','branch','contract_cost_yuan','emergency_cost_yuan','total_cost_yuan','final_energy_kwh','wall_seconds','optimizer_non_success_count','initializer_fallback_count']]),
      '',
      ...
      markdown(pairs.loc[pairs.purpose.str.startswith('Q43 '),['candidate','reference','total_saving_yuan','contract_saving_yuan','emergency_saving_yuan','winning_days','losing_days','losing_months','block7_lower_yuan','block7_upper_yuan','block14_lower_yuan','block14_upper_yuan']]),
      '',
      ...
      '',
      f"旧文件保护：{checks['preservation']['unchanged_protected_files']}个受保护文件逐字节不变，生产冻结10项和原附件1—4不变。例外为results/.DS_Store的macOS目录元数据SHA变化；未归因具体进程，无原始字节可恢复，前后SHA如实保留。该例外不影响论文/代码/工作簿/结果账本，但不能声称全部4219文件逐字节不变。",
      '', '## 文件、指定日期表与论文接力', '',
    ...
    result={'passed':True,'coverage':counts,'selected':chosen,'checks':{k:True for k in checks},'unresolved_external_assumptions':True,'research_not_blind_test':True,'new_policies':10,'workbooks':5,
        'legacy_research_artifacts_unchanged':not checks['preservation']['changed_research_files'],
        'all_prior_files_byte_identical':checks['preservation']['all_prior_files_byte_identical'],'metadata_exceptions':checks['preservation']['metadata_exceptions']}
    (ROOT/'acceptance.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n')
    lines=['# 最终验收','',markdown(pd.DataFrame([{'验收项':k,'结果':'PASS'} for k in checks])),'',
    ...
    error_rows=[]
    for tolerance,unit,label in [(1e-6,'kWh','年度物理/初始化最大误差'),(1e-5,'元','年度费用最大误差')]:
        values=[v['max_abs_error'] for v in checks['independent']['checks'] if v.get('tolerance')==tolerance and 'max_abs_error' in v]
        error_rows.append({'项目':label,'最大绝对误差':format(max(values),'.12g'),'单位':unit,'阈值':str(tolerance)})
    for key,unit,tol in [('kWh','kWh',1e-6),('yuan','元',1e-5)]:
        error_rows.append({'项目':'实际XLSX读回','最大绝对误差':format(checks['workbook']['max_abs_error'][key],'.12g'),'单位':unit,'阈值':str(tol)})
    error_rows.append({'项目':'非折点360坐标梯度','最大绝对误差':format(max(v['max_gradient_error_yuan_per_kwh'] for v in checks['numerical']['derivatives']),'.12g'),'单位':'元/kWh','阈值':'1e-5'})
    lines += ['', '## 最大误差与范围', '',markdown(pd.DataFrame(error_rows)), '',
        f"五工作簿检查{checks['workbook']['checks']:,}项，数值比较{checks['workbook']['numeric_comparisons']:,}项，错误0；覆盖50份CSV与50份Markdown指定日表、1008项时间映射、288720个采购单元格。三策略隔离复现的所有采购/候选/费用差为0。未来扰动16组的可用前输入及候选差为0。原始实际数据的单位和价格列在相应JSON单独列示。",
        '', '## 研究边界', '', '验收确认已执行的有限数据与实现，不确认正式题意、全局最优或未来年度表现。最终独立审阅见final_independent_review.md，九项模型评审见model_review.md，封包全部文件的双次SHA读回见package_readback.json。']

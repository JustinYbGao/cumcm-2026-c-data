...
def run_one(cfg, stage='runs', actual=None, archive=None):
    ...
    for i, day in enumerate(pd.date_range(cfg['start'], cfg['end'])):
        ...
        current['selected_index'] = k; current['selected_tau'] = np.repeat(chosen['tau_hours'],6)
        current['selected_profile'] = chosen['profile']
    ...
    frame[['date','slot_id','issue_time','forecast_history_end','load_forecast_kwh','pv_forecast_kwh',
           ...
           'selected_terminal_energy_kwh','selected_profile'] + PLAN_COLUMNS].to_csv(folder / 'plans.csv', index=False)
    ...
    daily['selected_profile'] = frame.groupby('date').selected_profile.first()
    ...
    np.savez_compressed(folder / 'decision_evidence.npz',
        ...
        score=np.stack([a['score'] for a in arrays]), cvar=np.stack([a['cvar'] for a in arrays]),
        feasible=np.stack([a['feasible'] for a in arrays]), ordinary=np.stack([a['ordinary'] for a in arrays]),
...
if __name__ == '__main__':
    ...
    configs = json.loads((WORK / 'configs/q2_pareto_v3' / ('extension.json' if args.extension else 'experiments.json')).read_text())['runs']
    allowed = ['T_rebalance','R_guard','R_mean','R_cost','R_w28'] if args.stage == 'january' and not args.extension else [c['policy'] for c in configs]
    if args.policies:
    ...
    for cfg in configs:
        ...
        run_one(cfg, ('extension_' if args.extension else '')+args.stage)

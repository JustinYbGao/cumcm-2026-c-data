...
fig.suptitle('Problem 2 | Monthly costs versus the seasonal baseline',fontsize=17,y=.985,x=.075,ha='left')
axes[0].bar(x,d.planned_cost_yuan/10000,width=.58,color='#a6cee3',label='Selected: planned cost')
axes[0].bar(x,d.emergency_cost_yuan/10000,width=.58,bottom=d.planned_cost_yuan/10000,
    color='#fdbf6f',label='Selected: emergency cost')
axes[0].plot(x,d.seasonal_total_cost_yuan/10000,'o-',color='#555555',lw=1.7,ms=4,label='Seasonal baseline: total cost')
axes[0].set(ylabel='Cost ($10^4$ CNY)',ylim=(0,245),yticks=[0,50,100,150,200])
axes[0].set_title('(a) Monthly cost breakdown',loc='left',fontsize=12,pad=9)
axes[0].legend(ncol=3,loc='upper left',fontsize=9.7,columnspacing=1.3)
axes[0].text(.02,.83,f'Feb-Dec total (million CNY): selected {total/100:.2f}  |  seasonal {baseline/100:.2f}',
...
axes[1].set(ylabel='Cost difference ($10^4$ CNY)',xlabel='Month (2025)',ylim=(-19,40),xticks=x,xticklabels=['Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'])
axes[1].set_title(f'(b) Selected minus seasonal: total +{(total-baseline)/100:.3f} million CNY (+{(total/baseline-1)*100:.2f}%)',loc='left',fontsize=12,pad=9)
for ax in axes:
...
fig.text(.075,.025,'Selected using January only; evaluated February-December. Positive gap = higher cost. Internal results.',fontsize=9.8,color='#59636f')
for ext in ['png','svg']:

...
def figure_audit():
    a = Audit(); metrics = {}
    visually_reviewed = {'savings': '5b1861fe27753da11e81773ac6b1348228c77dfaaf0f4469e52c08656ea831a1',
                         'monthly': 'b1602a3eacf6c94c7b518435e5fc20f611195e87465c6851d5f0b8bf94d3b612'}
    comparison = csv(ROOT/'analysis_all/comparison_all.csv').set_index('policy')
    months = csv(ROOT/'analysis_all/monthly.csv').set_index(['policy', 'month'])
    for name in ('savings', 'monthly'):
        folder = REPORT/'figures'/name; workspace = folder/'workspace'
        data = csv(folder/'data.csv')
        lineage = json.loads((workspace/'data_lineage.json').read_text())
        a.check(name+'.source_hash', digest(Path(lineage['source'])) == lineage['source_sha256'])
        a.check(name+'.data_hash', digest(folder/'data.csv') == lineage['plot_data_sha256'])
        if name == 'savings':
            policies = comparison.loc[comparison.stage == 'direct'].index.tolist()
            a.check(name+'.all_eight_policies', data.policy.tolist() == policies)
            for column in data.columns[1:]:
                a.close(name+'.mapping_'+column, data[column].to_numpy().astype(float), comparison.loc[policies, column].to_numpy().astype(float), CTOL)
        else:
            a.check(name+'.eleven_months', data.month.tolist() == pd.period_range('2025-02', '2025-12', freq='M').astype(str).tolist())
            for policy in data.columns[1:]:
                truth = months.loc['X_strong_morning85', 'total_cost_yuan']-months.loc[policy, 'total_cost_yuan']
                a.close(name+'.mapping_'+policy, data[policy].to_numpy(), truth.loc[data.month].to_numpy(), CTOL)
        raw = (folder/'outputs/chart.png').read_bytes(); offset = 8; dpi = None; dimensions = None
        a.check(name+'.png_signature', raw[:8] == b'\x89PNG\r\n\x1a\n')
        while offset < len(raw):
            ...
            if kind == b'IHDR':
                ...
                dpi = (x*.0254, y*.0254) if unit == 1 else None
            offset += size+12
        a.check(name+'.png_300dpi', dpi is not None and max(abs(value-300) for value in dpi) < .02)
        root = ET.parse(folder/'outputs/chart.svg').getroot()
        texts = [''.join(node.itertext()) for node in root.iter() if node.tag.endswith('}text')]
        a.check(name+'.svg_text_available', len(texts) > 10)
        a.check(name+'.svg_no_chinese_text', not any('\u3400' <= ch <= '\u9fff' for text in texts for ch in text))
        a.check(name+'.currency_units', any('thousand CNY' in text for text in texts))
        if name == 'savings':
            fields = ['ordinary_saving_vs_X_strong_morning85_yuan', 'emergency_saving_vs_X_strong_morning85_yuan', 'total_saving_vs_X_strong_morning85_yuan']
            for value in data[fields].to_numpy().ravel()/1000:
                a.check(name+'.signed_annotation_'+f'{value:+.2f}', f'{value:+.2f}' in texts)
        assessed = json.loads((workspace/'assistant_visual_assessment.json').read_text())
        a.check(name+'.visual_image_sha', assessed['inspected_png_sha256'] == digest(folder/'outputs/chart.png'))
        a.check(name+'.visual_script_sha', assessed['inspected_script_sha256'] == digest(workspace/'adapted_plot.py'))
        a.check(name+'.independent_actual_image_inspection', visually_reviewed[name] == digest(folder/'outputs/chart.png'))
        for filename in ('technical_quality_report.json', 'visual_quality_report.json', 'final_quality_report.json'):
            report = json.loads((workspace/filename).read_text())
            a.check(name+'.'+filename+'_passed', report['passed'])
        technical = json.loads((workspace/'technical_quality_report.json').read_text())
        a.check(name+'.technical_dimensions', dimensions == (technical['image_width'], technical['image_height']))
        a.check(name+'.technical_size', len(raw) == technical['file_size'])
        candidates = json.loads((workspace/'candidate_templates.json').read_text())
        selected = json.loads((workspace/'final_template_selection.json').read_text())
        a.check(name+'.selected_from_recalled_candidates', selected['selected_template_id'] in [c['template_id'] for c in candidates['candidates']])
        metrics[name] = dict(png_sha256=digest(folder/'outputs/chart.png'), svg_sha256=digest(folder/'outputs/chart.svg'),
            dimensions=list(dimensions), dpi=list(dpi), csv_rows=len(data), visible_svg_text_count=len(texts),
            actual_image_review='PASS: English labels, signed values and negative months retained; no visible clipping or overlap.',
            manual_repair_record_present=(workspace/'manual_repair_versions/change.json').exists())
    a.check('savings.initial_failure_retained', (REPORT/'figures/savings/workspace/initial_quality_failure.json').exists())
    return a, metrics
...
if __name__ == '__main__':
    ...
    functions = dict(oracle=oracle_audit, sources=sources_audit, references=references_audit,
                     ...
                     analysis_all=lambda: analysis_audit(True), figure_validation=figure_audit)

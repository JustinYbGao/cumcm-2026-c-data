...
def main():
    ...
    result=engine.WORK/'results/q2_direct_v4/analysis_all'
    ...
    for name,goal,keywords,types in specs:
        ...
        if name=='savings':
            f=raw.loc[raw.stage!='reference',['policy','ordinary_saving_vs_X_strong_morning85_yuan','emergency_saving_vs_X_strong_morning85_yuan','total_saving_vs_X_strong_morning85_yuan','emergency_cost_yuan','passes_improvement']]
        else:
            best=json.loads((result/'acceptance.json').read_text())['best_observed_policy']
            wide=raw.pivot(index='month',columns='policy',values='total_cost_yuan')
            f=pd.DataFrame({'month':wide.index})
            for policy in list(dict.fromkeys([best,'D112','D56_risk625','D56_mu0'])):f[policy]=(wide.X_strong_morning85-wide[policy]).to_numpy()
        f.to_csv(task/'data.csv',index=False)
        engine.save(ws/'data_lineage.json',{'source':str(source),'source_sha256':engine.sha(source),'plot_data_sha256':engine.sha(task/'data.csv'),'rows':len(f),'transform':'All eight new paths, or declared monthly paired comparisons with X_strong_morning85; exact subtraction and display unit conversion only.'})

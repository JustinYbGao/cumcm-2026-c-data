...
f=pd.read_csv(source,float_precision='round_trip');assert len(f)==8
...
fig.subplots_adjust(left=.205,right=.98,bottom=.20,top=.84,wspace=.18)
...
for i,(ax,col,title) in enumerate(zip(axes,fields,['(a) Ordinary purchase savings','(b) Emergency purchase savings','(c) Total savings'])):
    ...
    for j,b in enumerate(bars):
        if f.emergency_cost_yuan.iloc[j]>1e6:b.set_hatch('///')
    ...
    ax.set_xlabel('Savings vs v3 best (thousand CNY)',fontsize=10)
    ax.set_yticks(y,labels=f.policy.tolist());ax.grid(axis='x',color='white',linewidth=1.5,zorder=0)
    ax.tick_params(length=0)
    for spine in ax.spines.values():spine.set_visible(False)
axes[0].invert_yaxis()
fig.suptitle('Q2: direct procurement reduces total cost',fontsize=19,fontweight='bold',y=.97)
fig.text(.205,.917,'All eight new policies | 334-day historical replay | Positive values mean lower cost',fontsize=12)
fig.text(.205,.062,'Six initial policies and two solver-budget refinements. Hatched bars: emergency bill exceeds the 1,000,000 CNY cap.',fontsize=11)

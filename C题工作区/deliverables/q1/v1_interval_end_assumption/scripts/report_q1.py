...
def main():
    ...
    figures = WORK/'reports/figures/q1'
    figures.mkdir(parents=True,exist_ok=True)
    plt.rcParams.update({'font.size':10,'axes.spines.top':False,'axes.spines.right':False,'figure.dpi':150})
    fig,ax = plt.subplots(figsize=(10,4),layout='constrained')
    for f,label,color in [(p,'Each direction 90%','#2563eb'),(sens,'Round-trip 90%','#e58025')]:
        ax.plot(np.arange(145)/6,np.r_[f.energy_start_kwh.iloc[0],f.energy_end_kwh],label=label,color=color,lw=1.9)
    ax.axhline(1200,color='#888888',ls='--',lw=.8)
    ax.axhline(10800,color='#888888',ls='--',lw=.8)
    ax.set(xlim=(0,24),xticks=range(0,25,2),xlabel='Hour of day',ylabel='Stored energy (kWh)',
           title='Figure 1. Battery state — internal interval-end assumption')
    ax.grid(alpha=.2)
    ax.legend(loc='upper right')
    fig.savefig(figures/'battery_states.png')
    plt.close(fig)
    fig,axs = plt.subplots(3,1,figsize=(11,8),sharex=True,layout='constrained')
    x=p.start_minute/60
    axs[0].stairs(p.load_kw,np.arange(145)/6,label='Load',color='#374151',baseline=None)
    axs[0].stairs(p.pv_forecast_kw,np.arange(145)/6,label='Given PV forecast',color='#d9a514',baseline=None)
    axs[0].stairs(p.grid_kwh*6,np.arange(145)/6,label='Grid purchase',color='#2563eb',baseline=None)
    axs[0].set(ylabel='AC power (kW)',title='Figure 2. Baseline dispatch — internal interval-end assumption')
    axs[0].legend(ncol=3,loc='upper left')
    axs[1].bar(x,p.charge_kwh*6,width=1/6,align='edge',label='Charge (+)',color='#059669')
    axs[1].bar(x,-p.discharge_kwh*6,width=1/6,align='edge',label='Discharge (-)',color='#d85d52')
    axs[1].axhline(5000,color='#888888',ls='--',lw=.8)
    axs[1].axhline(-5000,color='#888888',ls='--',lw=.8)
    axs[1].set(ylabel='Battery AC power (kW)')
    axs[1].legend(ncol=2,loc='upper left')
    axs[2].stairs(p.price_yuan_per_kwh,np.arange(145)/6,color='#7c3aed',baseline=None)
    axs[2].set(xlim=(0,24),xticks=range(0,25,2),ylabel='Price (yuan/kWh)',xlabel='Hour of day')
    for ax in axs: ax.grid(alpha=.15)
    fig.savefig(figures/'dispatch.png')
    plt.close(fig)
...
![图1：内部工作假设下的储能曲线](figures/q1/battery_states.png)
...
![图2：基准调度与供需、电价](figures/q1/dispatch.png)
...
'''
    ...
    artifacts = list(root.rglob('*')) + list(figures.glob('*.png'))

...
For Problem 1, a deterministic mixed-integer linear program jointly schedules purchases and battery flows. HiGHS returns a daily cost of **35,126.95 CNY**, **26.898%** below the no-storage benchmark, with a reported zero optimality gap for that supplied instance.
...
For Problem 2, the reserve policy costs **13,845,452.99 CNY** and saves **69,508.07 CNY** against its matched greedy control. Emergency expenditure nevertheless rises by **22,702.84 CNY**. For Problem 3, the selected corrected-PV rolling reserve policy costs **13,574,461.81 CNY**. Within the reserve family, state feedback with paid revisions saves CNY 208,115.60; adding later raw PV releases saves CNY 167,121.93. The selected corrected-PV reserve policy costs an additional CNY 6,206.07 relative to its matched greedy control, so additional storage flexibility does not assure lower realized cost.

For Problem 4, planning prices are separated from actual billed prices. The midnight branch selects the causal-OLS midnight reserve policy, costing **14,618,518.98 CNY**; the rolling branch selects the raw-PV/causal-OLS rolling reserve policy, costing **14,313,637.10 CNY**. Both use actual variable-price settlement and the full fivefold emergency tariff.
...
**Keywords:** microgrid procurement; causal storage reserve; historical scenarios; paid rolling revisions; mixed-integer linear programming

# 1 Problem statement and modeling rationale

## 1.1 Decisions, costs, and required outputs
...
![Figure 1: Microgrid configuration and energy flows](figures/scene/outputs/chart.png)

*Figure 1. Microgrid configuration and energy flows. In Q2-Q4, emergency purchases cover demand left after discharge under the preissued storage reserve rule; they cannot charge storage. Unused surplus earns no export revenue. Arrows represent modeled energy flows, not detailed electrical wiring.*

**Q1: deterministic daily scheduling.** Attachment 1 supplies a daily load, tariff, and PV forecast profile. A purchase plan is made at midnight with equal beginning and ending stored energy. The task requires the specified interval purchases, daily purchase amount and fee, four-hour charging/discharging totals, and boundary energy in `result1.xlsx`.

**Q2: a fixed commitment under uncertain load and PV.** With the repeated Attachment 1 tariff and Attachment 2 observations, the entire normal-purchase vector is fixed at midnight. During actual operation, emergency purchases cover any remaining supply deficit at five times the trading-time tariff; the normal commitment remains payable. The annual plan and realized storage and emergency-purchase records form `result2.xlsx`.

**Q3: paid revision using released PV forecasts.** Attachment 3 adds 24-hour hourly PV forecasts released at 00:00, 06:00, 12:00, and 18:00. Future purchases may be revised using later releases. The statement assigns a 50%-of-tariff breach charge to reductions and a 1.5-times-tariff price to added quantities. The model must account for these charges and emergency purchases, produce `result3.xlsx`, and assess whether later forecasts justify revising the commitment.

**Q4: variable-price settlement.** Attachment 4 replaces the repeated tariff with time-varying actual prices. The Q2 and Q3 tasks are repeated as `result4-2.xlsx` and `result4-3.xlsx`. We interpret this extension as preserving each branch's PV access and revision permissions. Planning-price alternatives are compared under the same actual-price settlement within each branch.
...
## 1.2 Why these model components belong together
...
**Table 1. Task structure, model choice, and planned verification.**

| Task structure | Limitation requiring attention | Component and purpose | Evidence in this paper |
|---|---|---|---|
| Battery energy couples successive intervals; charge and discharge cannot coincide | Purchasing each interval's net deficit ignores storage transfer | Linear balance and energy recursion with binary operating modes | Q1 bounds, no-storage comparison, efficiency and power-side cases |
...
![Figure 2: Architecture of the five final policies](figures/storage_v6/outputs/F2_architecture.png)

*Figure 2. Common data and physical constraints feed an independent Q1 MILP and a shared historical-scenario purchase/reserve model for Q2-Q4. Historical-PV midnight policies and released-PV rolling policies have different information permissions. Q1 starts and ends at 6,000 kWh; annual policies carry their own actual state continuously without a daily terminal equality. Planning prices and billed prices remain distinct.*
...
# 2 Data, working interpretations, and notation

## 2.1 Data interfaces and information cutoffs
...
![Figure 3: Monthly distributions of daily demand, PV generation, and mean actual tariff](figures/front/F3_annual_ridges.png)

*Figure 3. Monthly distributions across all 365 supplied days of 2025. Daily demand and PV energy use all ten-minute observations (MWh/day); mean actual tariff is the daily arithmetic mean (CNY/kWh). Original daily observations and sample counts accompany each density curve; vertical marks are monthly medians. A Scott bandwidth computed from all 365 daily values is held fixed across months within each column, with a shared density-height scale. Months stay in calendar order, and zero-PV intervals remain in daily totals. Panel scales differ by physical quantity.*

Table 2 distinguishes admissible planning inputs from subsequent actual execution data. In the annual tasks, load forecasts are fixed at midnight. A forecast for a historical date is reconstructed with that date's own information cutoff, rather than with coefficients fitted later [4].

**Table 2. Available information and adopted purchase/reserve update schedule.**

| Branch | Midnight load/PV input | 06:00, 12:00, 18:00 | Planning price | Actual settlement |
|---|---|---|---|---|
...
## 2.2 Working interpretations and their consequences
...
**Table 3. Physical and accounting conventions used in the calculations.**

| Item | Adopted convention and source | Where it matters |
|---|---|---|
...
| Q4-2 PV information | Preserve Q2's historical-PV information set when recalculating it under variable prices; an author interpretation, not an explicit branch-specific ban on Attachment 3 | Q4-2 input construction and matched planning-price comparisons |
...
A common seasonal-baseline warm-up runs from 1 to 31 January, starting at the statement's 6,000 kWh. Its 1 January cold start uses zero normal planned purchases; subsequent January purchases follow that baseline. The retained 4,464-interval record ends at 7,268.4231640740745 kWh, which becomes the shared 1 February initial state (Appendix A). The compared annual policies branch from this state; it is not the outcome of running each final policy separately through January. Thereafter, each policy carries its own actual energy across every midnight and month boundary. Q1's equality of initial and terminal energy is required by its task. In Q2-Q4, a nominal zero-deficit trajectory and terminal energy of 1,200 kWh constrain the MILP initializer only. They do not constrain the final purchase/reserve pair, and actual storage is never reset to that level.
...
Time in (1) is measured in hours, giving kWh. Only issued values are used. A release contains 24 hours of forecasts, but the current optimization uses only intervals up to the current day's 24:00. The implications of this remaining-day horizon are discussed in Section 9.
...
| Symbol | Meaning | Unit |
|---|---|---|
| $d,t,k,s$ | Operating day; ten-minute interval $t=0,\ldots,143$; decision stage; completed historical scenario day | Index |
| $\tau_k,\mathcal T_k$ | First unexecuted boundary $\tau_k=36k$; remaining intervals $\{\tau_k,\ldots,143\}$ | Index, set |
| $H_{d,k}$ | Eligible completed historical days within the preceding $W$ calendar days | Set |
| $L_t,G_t$ | Community load energy and PV generation energy in interval $t$ | kWh |
| $\widehat L_{d,t,00}$ | Load-energy forecast for day $d$, interval $t$, fixed at midnight | kWh |
| $\widehat G_{d,t,k}$ | PV-energy forecast for day $d$, interval $t$, available at stage $k$ | kWh |
| $q_t^0$ | Original normal-purchase commitment made at midnight for interval $t$ | kWh |
| $q_t^k$ | Normal-purchase commitment for interval $t$ after stage $k$ | kWh |
| $\bar q_t$ | Final effective normal-purchase commitment executed in interval $t$ | kWh |
| $R_t^0,R_t^k,\bar R_t$ | Discharge reserve threshold issued at midnight, updated at stage $k$, and finally applied in interval $t$ | kWh |
| $c_t$ | Energy charged from the microgrid bus into the battery in interval $t$ | kWh |
| $b_t$ | Energy discharged from the battery to the microgrid bus in interval $t$ | kWh |
| $u_t$ | Emergency purchase covering the remaining supply deficit in interval $t$ | kWh |
| $w_t$ | Unused surplus disposed of without sale revenue in interval $t$ | kWh |
| $E_t$ | Stored energy at the beginning of interval $t$ | kWh |
| $p^{\mathrm{plan}}_{t\mid k},p^{\mathrm{act}}_t$ | Price used for planning; price used for actual settlement | CNY/kWh |
| $P$ | Bus-side charge/discharge energy limit per ten-minute interval, $5000/6$ | kWh |
| $\eta_c,\eta_d$ | Charging and discharging efficiencies, respectively | Dimensionless |
| $r^s_{t,k},n^s_{d,t,k}$ | Historical net forecast error; scenario net load | kWh |
| $W$ | Length of the calendar window used to select completed historical days | day |
| $\kappa$ | Emergency-cost weight in the empirical planning objective | Dimensionless |
| $\mu$ | Fixed terminal-inventory value proxy in the planning objective | CNY/kWh |
| $\nu$ | Common terminal-energy valuation used only in the historical inventory audit | CNY/kWh |
| $F_A$ | Final rule-A contract cost relative to the midnight commitment | CNY |
| $J_k$ | Remaining-horizon empirical planning objective at stage $k$ | CNY |
...
# 3 Common execution model and joint purchase/reserve objective

## 3.1 Physical execution and historical scenarios
...
## 3.3 Nominal initialization and candidate search
...
**Algorithm 1. Joint purchase/reserve decision at an authorized stage.**

1. Read the policy's actual energy and available information. Freeze both purchases and reserve thresholds for executed intervals.
2. Form release-matched historical scenarios using (4). Every candidate starts from the same current actual energy.
3. Construct a risk-profile MILP purchase start and a positive point-net-load start. Run two purchase-only L-BFGS-B searches with $R=1200$, retaining each start, best evaluated point, and returned point: six greedy candidates.
4. Start two additional searches from the risk-profile purchase vector and the best of those six purchase vectors. In the reserve arm, their reserve starts are the MILP's nominal end-of-interval energies clipped to $[1200,10800]$, and the constant 1,200 vector, respectively. Use the constant vector if the initializer has no energy trajectory. In the matched greedy arm, both additional searches keep $R=1200$.
5. Retain each additional start, best evaluated pair, and returned pair. Rescore all twelve candidates with (5). At an intraday update, append the complete unchanged current pair $(q,R)$ as candidate thirteen. Select the first score within $10^{-8}$ CNY of the minimum in this fixed order.
6. Apply the selected future purchases and thresholds, execute (3) until the next allowed decision, and carry actual energy forward. Settle final effective purchase quantities once using (7).
...
The adopted parameters remain $W=112$, $\kappa=5$, and $\mu=0.38232$, inherited from the earlier Q2 study without retuning. Every search uses `maxiter=180`, `maxfun=1600`, `maxls=30`, `maxcor=10`, `ftol=1e-10`, and `gtol=1e-5`. Procurement coordinates are scaled as $q/1000$, reserve coordinates as $(R-1200)/1000$, and the objective as $J/10000$. Joint searches use derivatives with respect to both vectors; rule A changes only the procurement part of the gradient. The compiled forward and backward recursion evaluates the same model. At nonsmooth ties it uses recorded branch derivatives, including the intermediate contract slope $p$ at $q=q^0$; these choices do not imply differentiability. Appendix A identifies code and configuration records.
...
# 4 Q1: Deterministic daily storage scheduling

## 4.1 Deterministic formulation and results
...
**Table 5. Q1 daily storage comparison; bus-side energy quantities.**

| Configuration | Cost (CNY) | Purchase (kWh) | Charge (kWh) | Discharge (kWh) | Curtailed PV (kWh) |
|---|---:|---:|---:|---:|---:|
...
Tables 19-20 in Appendix D.1 give the prescribed Q1 outputs from `result1.xlsx`; Table 18 maps each workbook to the statement requirements.

## 4.2 Physical interpretation sensitivity and checks
...
**Table 6. Q1 efficiency-by-power-side sensitivity.**

| Efficiency convention | Power rating applies at | Cost (CNY) |
|---|---|---:|
...
# 5 Problem 2: History-Based Direct Procurement

## 5.1 Forecast inputs and historical scenarios
...
## 5.2 Matched costs, selection, and historical variability
...
**Table 7. Q2 matched controllers and frozen historical references, February-December 2025 (CNY).**

| Policy / controller | Contract cost | Emergency cost | Total cost |
| --- | --- | --- | --- |
...
Q2's prescribed-date results and emergency records are in Appendix D.2 (Tables 21-24) and `result2.xlsx`.

![Figure 5: Q2 reserve-policy monthly savings and 7/14-day descriptive circular-block intervals against matched greedy and frozen D112 references](figures/storage_v6/outputs/F5_q2_monthly.png)

*Figure 5. Q2 reserve-policy monthly savings and 7/14-day descriptive circular-block intervals against matched greedy and frozen D112 references. Negative months and any intervals crossing zero remain visible; the two references answer different comparison questions.*
...
# 6 Problem 3: Paid Rolling Procurement with Updated PV Forecasts

## 6.1 Rolling decisions and information comparisons

Problem 3 adds forecasts released at 00:00, 06:00, 12:00 and 18:00 and permits paid procurement revisions after midnight. We retain Q2's shared purchase/reserve objective, execution rule, and fixed parameters, using the current Attachment 3 forecast and the final-contract cost from Section 3. The load forecast is formed at midnight and remains fixed throughout the day. A PV release is interpolated onto the ten-minute grid and truncated at the current day's midnight boundary. Although each release extends 24 hours ahead, this inherited optimization horizon ends at 24:00 of the present day.
...
![Figure 6: Q3 information comparisons within each storage-controller family](figures/storage_v6/outputs/F6_q3_information.png)

*Figure 6. Q3 information comparisons within each storage-controller family. Signed contract and emergency savings distinguish state feedback with revisions from the additional effect of later raw PV releases. Each comparison uses 334 matched dates.*
...
The selected Q3 schedule is detailed in Appendix D.3, Tables 25-28, and `result3.xlsx`, including its original and revised purchases.

# 7 Problem 4: Planning Prices and Realized Variable-Price Settlement

## 7.1 Price information without advance knowledge of actual tariffs
...
## 7.2 Price comparisons and selected policies
...
Appendix D.4, Tables 29-32, and `result4-2.xlsx` contain the prescribed-date outputs for the selected Q4-2 policy.

![Figure 7: Q4 planning-price and PV-correction comparisons under matched storage controllers](figures/storage_v6/outputs/F8_q4_comparisons.png)

*Figure 7. Q4 planning-price and PV-correction comparisons under matched storage controllers. Q4-2 retains historical PV; Q4-3 uses the permitted forecast releases. Every cost is billed at actual variable prices, regardless of its planning-price input.*

Figure 7 compares the price and PV-correction choices. With raw PV, causal OLS saves CNY 28,892.88 in the reserve arm. Under OLS, PV correction costs an additional CNY 4,984.88; under fixed planning prices it saves CNY 230.32. The selected reserve policy is `q43_raw_ols_reserve`, the raw-PV/causal-OLS rolling reserve policy. It saves CNY 7,194.43 against the matched greedy configuration. Table 15 preserves all eight outcomes so price effects are not inferred from differently corrected PV inputs or controllers.
...
The corresponding Q4-3 outputs, including paid revisions and emergency events, are in Appendix D.5, Tables 33-36, and `result4-3.xlsx`.
...
![Figure 8: Selected Q4-3 policy on 14 July 2025: released planning prices, purchase versions, and storage energy with executed reserve thresholds](figures/storage_v6/outputs/F9_q43_causal_trace.png)

*Figure 8. Selected Q4-3 policy on 14 July 2025: released planning prices, purchase versions, and storage energy with executed reserve thresholds. Future versions begin only at 00:00, 06:00, 12:00, or 18:00. Actual prices are retrospective settlement data, and final normal purchases exclude emergencies.*
...
# 8 Correctness, sensitivity, and historical uncertainty

## 8.1 Implementation, information cutoffs, and reproduction
...
**Table 9. Recorded v6 verification evidence and tolerances.**

| Object | Scope of comparison | Maximum absolute discrepancy | Threshold |
|---|---|---:|---:|
...
![Figure 9: Midnight and latest available forecast distributions against actual PV energy and electricity prices](figures/advanced/F10_forecast_joint.png)

*Figure 9. Forecast-actual distributions for the selected raw-PV/OLS Q4-3 reserve policy, February-December 2025. PV panels use the same actual-positive subset (26,880 intervals); price panels use all 48,096 intervals. Forecast versions are aligned by date, interval, and permitted issue time. Hexagons show counts on a shared logarithmic scale within each row; marginals show counts, dashed lines show equality, and circles mark the 30 largest absolute errors per panel. Bias is forecast minus actual. Actual values are retrospective evaluation outcomes.*

## 8.2 Paired savings, losing periods, and terminal inventory

For candidate $a$ and reference $b$, daily saving is $S_d=B_d^b-B_d^a$. Positive values favor the candidate; monthly and annual savings sum these same matched daily differences. The denominator of a saving percentage is the named reference's total cash bill. Each selected reserve policy is paired below with the same input configuration in the four-search greedy arm.
...
![Figure 10: Matched daily savings distributions for the four selected reserve policies](figures/advanced/F11_daily_savings_raincloud.png)

*Figure 10. Daily cash savings relative to each selected policy's same-configuration four-search greedy control, February-December 2025. Each row contains all 334 days. Positive values favor reserve. Three adjoining horizontal ranges retain both tails while enlarging the center; their scales differ. Half-violins use Scott-bandwidth density estimates normalized to each row's peak and show shape only; plotted areas and heights are not probability comparisons. Boxes show the median and interquartile range, with 1.5-IQR whiskers; dots are original daily values with vertical jitter. Annual net savings and winning-day counts use the full rows.*

**Table 10. Selected reserve policies versus same-configuration greedy controls, 334 days; CNY. Descriptive 95% intervals use 2,000 circular-block resamples.**

| Branch | Matched greedy reference | Cash saving | 7-day interval | 14-day interval |
| --- | --- | --- | --- | --- |
| Q2 | q2_greedy | 69,508.07 | [4,484.52, 160,892.55] | [3,279.77, 162,427.06] |
| Q3 | q3_w28_greedy | -6,206.07 | [-26,367.00, 14,137.80] | [-29,281.12, 16,720.13] |
| Q4-2 | q42_ols_greedy | 68,043.65 | [1,453.28, 155,787.03] | [-3,135.99, 158,176.86] |
| Q4-3 | q43_raw_ols_greedy | 7,194.43 | [-14,347.68, 30,167.31] | [-18,423.91, 33,626.59] |

Both Q2 block intervals are positive. Q4-2's 14-day interval crosses zero, and both intervals cross zero for Q3 and Q4-3. Every selected reserve policy has losing months against its matched greedy control; favorable annual totals do not imply uniform benefit.

Figure 11 extends the monthly comparison to all eleven matched configurations. Color is the monthly cash saving as a percentage of that configuration's greedy bill, so it is not an absolute-cost ranking. Rows are ordered by average-linkage Euclidean clustering of their standardized monthly percentage profiles; the displayed colors remain unstandardized and months remain chronological. The side tracks show annual cash saving and additional emergency energy, making the cost and emergency-use tradeoff visible. These are comparisons of the complete procurement-and-storage procedures, not isolated effects of changing a reserve threshold while holding procurement fixed.

![Figure 11: Monthly savings profiles, annual net savings, and additional emergency energy across all eleven matched configurations](figures/advanced/F12_seasonal_fingerprint.png)

*Figure 11. Seasonal savings profiles across all eleven reserve/greedy pairs, February-December 2025. Heatmap values are 100 times the monthly greedy-minus-reserve cash difference divided by the matched greedy monthly bill. Standardization affects the descriptive tree and row order only. Side tracks show annual net saving (thousand CNY) and reserve-minus-greedy emergency energy (MWh); diamonds mark the four selected reserve policies. Q4 fixed/OLS labels identify planning prices, while bills use actual prices. Hatching marks negative monthly savings.*
...
**Table 11. Terminal-inventory audit for the same matched comparisons; energy in kWh and savings in CNY.**

| Branch | Reserve final E | Greedy final E | Cash saving | Inventory-adjusted |
| --- | --- | --- | --- | --- |
...
**Table 12. Five final policies: cash fees in CNY and final stored energy in kWh. Q1 is one day; annual branches cover 334 days.**

| Branch | Decision / PV input | Planning price | Total fee | Emergency fee | Final energy |
| --- | --- | --- | --- | --- | --- |
...
[1] CUMCM Organizing Committee. *2026 CUMCM Problem C: Microgrid and External-Grid Power Regulation Strategies*, with Attachments 1-5. Supplied competition statement and data, 2026.

[2] Sigalo, M. B., Pillai, A. C., Das, S., and Abusara, M. An Energy Management System for the Control of Battery Storage in a Grid-Connected Microgrid Using Mixed Integer Linear Programming. *Energies*, 14(19), 6212, 2021. [doi:10.3390/en14196212](https://doi.org/10.3390/en14196212).

[3] Parisio, A., Rikos, E., and Glielmo, L. Stochastic model predictive control for economic/environmental operation management of microgrids: An experimental case study. *Journal of Process Control*, 43, 24-37, 2016. [doi:10.1016/j.jprocont.2016.04.008](https://doi.org/10.1016/j.jprocont.2016.04.008).

[4] Hyndman, R. J., and Athanasopoulos, G. *Forecasting: Principles and Practice*, 3rd ed. OTexts, 2021. Section 5.10, Time series cross-validation, accessed 11 September 2026. [Chapter](https://otexts.com/fpp3/tscv.html).

[5] Kleywegt, A. J., Shapiro, A., and Homem-de-Mello, T. The Sample Average Approximation Method for Stochastic Discrete Optimization. *SIAM Journal on Optimization*, 12(2), 479-502, 2002. [doi:10.1137/S1052623499363220](https://doi.org/10.1137/S1052623499363220).

[6] Byrd, R. H., Lu, P., Nocedal, J., and Zhu, C. A Limited Memory Algorithm for Bound Constrained Optimization. *SIAM Journal on Scientific Computing*, 16(5), 1190-1208, 1995. [doi:10.1137/0916069](https://doi.org/10.1137/0916069).

[7] SciPy developers. *minimize(method='L-BFGS-B')*. Official API documentation, accessed 11 September 2026. [Method documentation](https://docs.scipy.org/doc/scipy/reference/optimize.minimize-lbfgsb.html).

[8] HiGHS developers. *HiGHS: Linear Optimization Software*. Official project and documentation, accessed 11 September 2026. [HiGHS](https://highs.dev/).

[9] Sheppard, K. *arch: Time-series Bootstraps*. Official software documentation, accessed 11 September 2026. [Circular and other time-series bootstraps](https://bashtage.github.io/arch/bootstrap/timeseries-bootstraps.html).

# Appendix A. Implementation and reproducibility specification

Appendix E contains the complete scientific source listings and the exact support-file inventory. In the accompanying support archive, `README.md` gives the environment, original-input placement, lightweight checks, and optional reproduction commands; `MANIFEST.csv` records packaged identities. The scientific entry points are `scripts/solve_q1.py` and `scripts/storage_control_v6/run.py`. The manuscript builder only typesets completed evidence. The twenty-two annual experiments and their isolated reproduction were performed before this editorial revision.

Paths in Table 13 are relative to the scientific support directory unless explicitly labeled as original project provenance. Original contest attachments are not duplicated in the archive: `raw/required_files.json` identifies them, and `raw/README.md` specifies neutral input filenames. `SOURCE_PROVENANCE.json` distinguishes original sources, portable copies and saved evidence; `source_changes.diff` records path and logging adaptations. The historical studies remain named references and are not represented as newly executed results.

**Table 13. Scientific program, saved-result, and provenance interfaces.**

| Model or evidence | Relative entry | Role |
|---|---|---|
| Raw inputs and conversion | `raw/README.md`, `raw/required_files.json`; `scripts/prepare_data.py`, `scripts/prepare_inputs.py`; `configs/data_processing.json` | Required original-input identities, key alignment and forecast conversion |
| Q1 schedule and diagnostics | `scripts/solve_q1.py`; `saved/q1/` | Complete deterministic model, saved baseline, certificate and named physical cases |
| Forecast family and common January state | `scripts/run_q2.py`; `scripts/storage_control_v6/compat.py`; `saved/warmup/` | Original forecasting and initialization logic; actual common boundary record |
| Current annual model | `scripts/storage_control_v6/`: `inputs.py`, `policy.py`, `control.py`, `kernel.py`, `kernel.c`, `run.py`, `compat.py` | Causal inputs, shared candidates, reserve execution and settlement |
| Current configurations and analysis | `configs/storage_control_v6/experiments.json`; `scripts/storage_control_v6/analyze.py`; `saved/comparisons/` | All controller configurations, restricted selection and named comparisons |
| Selected issued and executed decisions | `saved/selected/<policy>/`: `execution_plans.npz`, `plan_versions.npz`, `ledger_four_dates.csv`, `plan_versions_four_dates.csv` | Lossless annual purchase/reserve arrays and complete prescribed-date records |
| All current outcomes | `saved/all_policies/` | Every current daily summary, configuration and total, including adverse alternatives |
| Frozen comparison references | `saved/historical_references/`; `results/q2_direct_v4/runs/D112/`; `results/unified_direct_v5/runs/` | Original reference summaries and date/cash/state projections for analysis |
| Historical development provenance | Original project: `results/q2_direct_v4/analysis_all/comparison_all.csv`, `results/innovation/evaluation/`, `results/robustness/` | Retained sources of Tables 14, 16 and 17; separate historical methods and periods |
| Independent saved-result checks | `scripts/check_saved.py`, `scripts/smoke_test.py` | Workbook readback, physical/cash replay and bounded execution checks; no annual optimization in lightweight commands |
| Final workbooks | `workbooks/`: `result1.xlsx`, `result2.xlsx`, `result3.xlsx`, `result4-2.xlsx`, `result4-3.xlsx`, `policy_mapping.json` | Unchanged English results and original template/policy mapping |
| Environment and identities | `requirements.txt`, `MANIFEST.csv`, `SOURCE_PROVENANCE.json`, `source_changes.diff` | Dependencies, actual archive inventory and traceable source adaptations |

The candidate order is profile start/best/return, point start/best/return, additional profile start/best/return, and additional best-greedy start/best/return. Intraday, `keep_current` is last and retains both future vectors. The first candidate within $10^{-8}$ CNY of the rescored minimum is selected. The nominal initializer constrains only its own trajectory; its end-of-interval energy values initialize one reserve search. Section 3.3 gives all search limits and scaling. There is no random scenario subsampling.

The independent checker in Section 8 rebuilds raw inputs, historical forecast parameters, and scenario scores from saved candidates using implementations separate from the production forecast, execution, billing, and objective functions. The isolated reproduction reruns the production code in a separate source/data tree and recompiles the kernel. Both studies predate this editorial revision. The bundled `check_saved.py` supports saved-result checks, with default and optional full-input replay modes documented in the support README.
...
**Table 14. Complete Q2 historical comparison, 1 February-31 December 2025.**

| Policy ID | Ordinary (CNY) | Emergency (CNY) | Total (CNY) | Final E (kWh) |
| --- | --- | --- | --- | --- |
| B0 | 11,987,831.57 | 4,345,851.83 | 16,333,683.39 | 2,723.620153 |
| B1 | 12,263,657.58 | 3,114,549.77 | 15,378,207.35 | 6,434.175697 |
| P_floor | 13,361,564.95 | 661,273.65 | 14,022,838.60 | 1,978.550864 |
| X_strong_morning85 | 13,349,990.09 | 644,292.17 | 13,994,282.26 | 2,441.729981 |
...
| D56_risk625 | 13,305,891.73 | 638,539.01 | 13,944,430.74 | 2,060.384331 |
| D56_mu069 | 13,294,245.37 | 734,215.87 | 14,028,461.25 | 10,727.473563 |
| D56_mu0 | 13,152,123.53 | 920,040.32 | 14,072,163.85 | 1,569.804649 |
| D112_refined | 13,075,951.33 | 839,116.05 | 13,915,067.38 | 3,154.980179 |
| D56_risk625_refined | 13,305,422.99 | 643,672.76 | 13,949,095.75 | 2,054.975640 |

## B.2 Complete current matched matrix
...
**Table 15. All 22 matched v6 policies, 1 February-31 December 2025; CNY. Suffixes identify the controller.**

| Policy ID | Contract | Emergency | Total | Non-success starts |
| --- | --- | --- | --- | --- |
| q2_greedy | 13,075,508.77 | 839,452.30 | 13,914,961.06 | 952 |
| q2_reserve | 12,983,297.86 | 862,155.13 | 13,845,452.99 | 1,071 |
| q3_no_update_greedy | 13,352,912.18 | 640,123.32 | 13,993,035.50 | 930 |
| q3_no_update_reserve | 13,264,208.54 | 686,026.42 | 13,950,234.96 | 1,043 |
| q3_soc_greedy | 13,428,635.70 | 275,573.74 | 13,704,209.45 | 2,539 |
| q3_soc_reserve | 13,313,835.92 | 428,283.44 | 13,742,119.36 | 3,002 |
| q3_raw_greedy | 13,260,330.46 | 298,999.92 | 13,559,330.38 | 2,571 |
| q3_raw_reserve | 13,201,668.27 | 373,329.17 | 13,574,997.43 | 2,969 |
| q3_w28_greedy | 13,286,743.84 | 281,511.90 | 13,568,255.74 | 2,497 |
| q3_w28_reserve | 13,234,894.79 | 339,567.02 | 13,574,461.81 | 2,935 |
| q42_ols_greedy | 13,817,869.87 | 868,692.76 | 14,686,562.63 | 1,018 |
| q42_ols_reserve | 13,704,872.50 | 913,646.48 | 14,618,518.98 | 1,108 |
| q42_fixed_greedy | 13,808,029.62 | 871,746.10 | 14,679,775.73 | 952 |
| q42_fixed_reserve | 13,708,867.43 | 912,730.83 | 14,621,598.25 | 1,071 |
| q43_raw_ols_greedy | 13,994,684.40 | 326,147.12 | 14,320,831.53 | 2,394 |
| q43_raw_ols_reserve | 13,923,929.42 | 389,707.68 | 14,313,637.10 | 2,964 |
| q43_w28_ols_greedy | 14,017,133.20 | 306,013.76 | 14,323,146.95 | 2,423 |
| q43_w28_ols_reserve | 13,957,964.46 | 360,657.52 | 14,318,621.98 | 2,954 |
| q43_raw_fixed_greedy | 14,004,935.62 | 327,290.75 | 14,332,226.37 | 2,571 |
| q43_raw_fixed_reserve | 13,943,230.62 | 399,299.36 | 14,342,529.98 | 2,969 |
| q43_w28_fixed_greedy | 14,031,875.36 | 310,663.35 | 14,342,538.71 | 2,497 |
| q43_w28_fixed_reserve | 13,977,052.49 | 365,247.17 | 14,342,299.66 | 2,935 |

## B.3 Earlier reserve and update-gate experiments
...
**Table 16. Earlier candidate outcomes, 1 April-31 December 2025; saving = within-group reference minus candidate.**

| Policy ID | Group reference | Total (CNY) | Saving (CNY) | Emergency (kWh) |
| --- | --- | --- | --- | --- |
| pv_raw | pv_raw | 11,438,135.24 | 0.00 | 115,781.783236 |
| pv_bias | pv_raw | 11,391,988.76 | 46,146.49 | 107,231.288066 |
| risk_none | risk_none | 13,750,367.53 | 0.00 | 676,738.389457 |
| risk_fixed | risk_none | 13,750,362.50 | 5.03 | 676,735.970580 |
| risk_g05 | risk_none | 13,750,362.50 | 5.03 | 676,735.970580 |
| risk_g10 | risk_none | 13,750,365.17 | 2.36 | 676,735.970580 |
| gate_paid | gate_paid | 11,438,052.17 | 0.00 | 115,748.103000 |
| gate_q50 | gate_paid | 11,440,332.16 | -2,279.99 | 119,018.242572 |
| gate_q75 | gate_paid | 11,573,630.89 | -135,578.73 | 145,217.448236 |
| gate_frozen | gate_paid | 13,239,531.48 | -1,801,479.31 | 507,500.070395 |
| gate_info_frozen | gate_paid | 13,239,531.48 | -1,801,479.31 | 507,500.070395 |
...
## B.4 Earlier correction sensitivity
...
**Table 17. Earlier correction sensitivity, 1 April-31 December 2025; CNY.**

| Raw / corrected pair | Raw cost | Corrected cost | Cash saving | Inventory-adjusted |
| --- | --- | --- | --- | --- |
| fixed_raw / fixed_w14 | 11,438,135.24 | 11,401,468.18 | 36,667.07 | 36,613.71 |
| fixed_raw / fixed_w28 | 11,438,135.24 | 11,391,988.76 | 46,146.49 | 46,106.35 |
| fixed_raw / fixed_w56 | 11,438,135.24 | 11,389,342.27 | 48,792.97 | 48,752.49 |
| efficiency_raw / efficiency_w28 | 11,055,054.93 | 11,004,126.31 | 50,928.63 | 50,857.90 |
| soft_raw / soft_w28 | 11,438,052.17 | 11,391,863.74 | 46,188.42 | 46,192.08 |
| variable_raw / variable_w28 | 12,142,444.11 | 12,100,661.01 | 41,783.10 | 41,783.13 |
...
# Appendix C. Workbook, table, and audit mapping

The support archive contains the five current workbooks in `workbooks/`. They are byte-identical copies of the English files under the original project directory `outputs/storage_control_v6_english_20260912/`; original production sources remain under `outputs/storage_control_v6/`. The package manifests identify these roles separately. English workbook copies preserve the numerical results and physical time intervals of the verified production sources, with translated labels recorded by the accompanying mapping.

**Table 18. Final workbooks and original statement requirements.**

| Workbook | Actual policy identifier | Saved record directory | Required response |
| --- | --- | --- | --- |
| `result1.xlsx` | `baseline` | `saved/q1/baseline/` | Q1 original Tables 1-2 |
| `result2.xlsx` | `q2_reserve` | `saved/selected/q2_reserve/` | Q2 original Tables 1-3, four dates |
| `result3.xlsx` | `q3_w28_reserve` | `saved/selected/q3_w28_reserve/` | Q3 original Tables 1-3, four dates |
| `result4-2.xlsx` | `q42_ols_reserve` | `saved/selected/q42_ols_reserve/` | Q4-2 original Tables 1-3, four dates |
| `result4-3.xlsx` | `q43_raw_ols_reserve` | `saved/selected/q43_raw_ols_reserve/` | Q4-3 original Tables 1-3, four dates |
...
The saved-record paths in Table 18 are relative to the support archive. Annual directories include `ledger_four_dates.csv` and `plan_versions_four_dates.csv`; Appendix D reports their complete required aggregates and emergency-event lists. Q1 uses the saved baseline schedule and states.

Official templates have no reserve-threshold column. Annual issued purchase/reserve versions are therefore retained losslessly in `plan_versions.npz`, and the midnight/final purchase and executed threshold arrays in `execution_plans.npz`. Together with original actual inputs, these reconstruct the selected energy paths without adding columns to the required worksheets. The original workspace archives also retain every discarded candidate and scenario input; those larger candidate archives are not part of the compact support submission.
...
# Appendix D. Complete prescribed outputs
...
## D.1 Q1 prescribed results

Tables 19-20 report `baseline` in `result1.xlsx`. The supplied deterministic day has no emergency purchase or contract revision.

**Table 19. Specified purchases, daily purchase and fee.**

| Interval / scope | Quantity | single day |
| --- | --- | --- |
...
**Table 20. Six four-hour storage blocks and start/end stored energy.**

| Period / instant | Quantity | single day |
| --- | --- | --- |
...
## D.2 Q2 prescribed results

Tables 21-24 report `q2_reserve` in `result2.xlsx`. All four required dates are retained: 20 March, 21 June, 23 September, and 21 December 2025.

**Table 21. Q2 specified purchases and daily energy totals; selected reserve policy.**

| Interval / scope | Quantity | 2025-03-20 | 2025-06-21 | 2025-09-23 | 2025-12-21 |
| --- | --- | --- | --- | --- | --- |
...
**Table 22. Q2 daily fees (CNY), independently recomputed from final quantities.**

| Interval / scope | Fee component | 2025-03-20 | 2025-06-21 | 2025-09-23 | 2025-12-21 |
| --- | --- | --- | --- | --- | --- |
...
**Table 23. Q2 six four-hour storage blocks and boundary energies.**

| Period / instant | Quantity | 2025-03-20 | 2025-06-21 | 2025-09-23 | 2025-12-21 |
| --- | --- | --- | --- | --- | --- |
...
**Table 24. Q2 complete emergency-event listing for the four specified dates.**

| Date | Event status | Interval | Emergency energy (kWh) |
| --- | --- | --- | --- |
...
## D.3 Q3 prescribed results

Tables 25-28 report `q3_w28_reserve` in `result3.xlsx`. All four required dates are retained: 20 March, 21 June, 23 September, and 21 December 2025.

**Table 25. Q3 specified purchases and daily energy totals; selected reserve policy.**
...
**Table 26. Q3 daily fees (CNY), independently recomputed from final quantities.**
...
**Table 27. Q3 six four-hour storage blocks and boundary energies.**
...
**Table 28. Q3 complete emergency-event listing for the four specified dates.**
...
## D.4 Q4-2 prescribed results

Tables 29-32 report `q42_ols_reserve` in `result4-2.xlsx`. All four required dates are retained: 20 March, 21 June, 23 September, and 21 December 2025.

**Table 29. Q4-2 specified purchases and daily energy totals; selected reserve policy.**
...
**Table 30. Q4-2 daily fees (CNY), independently recomputed from final quantities.**
...
**Table 31. Q4-2 six four-hour storage blocks and boundary energies.**
...
**Table 32. Q4-2 complete emergency-event listing for the four specified dates.**
...
## D.5 Q4-3 prescribed results

Tables 33-36 report `q43_raw_ols_reserve` in `result4-3.xlsx`. All four required dates are retained: 20 March, 21 June, 23 September, and 21 December 2025.

**Table 33. Q4-3 specified purchases and daily energy totals; selected reserve policy.**
...
**Table 34. Q4-3 daily fees (CNY), independently recomputed from final quantities.**
...
**Table 35. Q4-3 six four-hour storage blocks and boundary energies.**
...
**Table 36. Q4-3 complete emergency-event listing for the four specified dates.**
...
# Appendix E. Complete scientific programs
